...
@TeleOp(name="LServo", group="BionicBot")
public class STest extends LinearOpMode {
    BNO055IMU imu;

    Orientation angles;
    Orientation newAngles;
    LaunchServoTest robot = new LaunchServoTest();   // Use a Pushbot's hardware
    ...
    public void runOpMode() {
       ...
       //robot.shootSlider.setMode(DcMotor.RunMode.RUN_USING_ENCODER);

    waitForStart();
        while (opModeIsActive())
        {
        //robot.rotServo.setPower(1);
    ...
    public void TurnShootSlider() {


        //robot.shootSlider.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);

       // robot.shootSlider.setTargetPosition(robot.shootSlider.getCurrentPosition() + 113);

        //robot.shootSlider.setMode(DcMotor.RunMode.RUN_TO_POSITION);

        robot.shootSlider.setPower(1);

        while (robot.shootSlider.isBusy()) {
